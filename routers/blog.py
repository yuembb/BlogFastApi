...
def get_date_list(check_staff:Staff=Depends(token_check)):
    ...
    check_record = fetchall__dict2dot(cursor,f'''
    select
    b.content ->>'title' as blog_title,
    b.content ->>'contents' as blog_content,
    b.content ->>'created_date' as blog_created_date,
    ...
    inner join
    category c on(b.content->>'category_id')::int = c.id
    order by b.content->>'created_date' ;''')
...
@router.delete("/delete",summary="blog silme")
def delete_blog(blog_id:int,check_staff:Staff=Depends(token_check)):
    staff_check , connection,cursor = check_staff
    if not staff_check: return JSONResponse(status_code=401, content={"status":False,"message":"staff_not_found"})

    check_record = fetchone__dict2dot(cursor,f'''select * from blog;''')
    if not check_record:
        connection_close(connection,cursor)
        return {"status":"blog_not_found"}

    cursor.execute(f'''delete from blog where id ={blog_id}    ;''')
    commit__connection_close(connection,cursor)
    return {"status":"deleted"}

# @router.post("/create",summary="blog ekleme")
# def add_blog(create_blog:CreateBlog,chekc_staff:Staff=Depends(token_check)):
#     staff_check , connection,cursor = chekc_staff
#     if not staff_check: JSONResponse(status_code=401, content={"status":False,"message":"staff_not_found"})
#
#     content = create_blog.content
#
#     cursor.execute(f'''insert into blog (content) values ({Json(content)})   ;''')
#     commit__connection_close(connection,cursor)
#     return {"status":"created"}
#
#
# @router.put("/update",summary="update blog")
# def update_blog(blog_id:int,blog_info:UpdateBlog,check_staff:Staff=Depends(token_check)):
#     staff_check , connection,cursor = check_staff
#     if not staff_check: JSONResponse(status_code=401, content={"status":False,"message":"staff_not_found"})
#
...
#     incoming_data = blog_info.content
#
#     new_data = check_record.content
#
#     new_data["title"]=incoming_data["title"]
#     new_data["contents"]=incoming_data["contents"]
#     new_data["staff_id"]=incoming_data["staff_id"]
#     new_data["category_id"]=incoming_data["category_id"]
#     new_data["created_date"]=incoming_data["created_date"]
#
#     cursor.execute(f'''update blog set content=({Json(new_data)}) where id = {blog_id}  ;''')
#     commit__connection_close(connection,cursor)
#     return {"status":"updated"}
#
# @router.get("/gets",summary="gets blog")
# def gets_blog(check_staff:Staff=Depends(token_check)):
#     staff_check, connection, cursor = check_staff
#     if not staff_check: JSONResponse(status_code=401, content={"status": False, "message": "staff_not_found"})
#
#     check_record = fetchall__dict2dot(cursor,f'''select * from blog;''')
#     if check_record:
#         return check_record
#
#     return {"status": "blog_not_found"}
...
#
#
# @router.get("/list",summary="all list")
# def list_all(check_staff:Staff=Depends(token_check)):
#     staff_check, connection, cursor = check_staff
#     if not staff_check: JSONResponse(status_code=401, content={"status": False, "message": "staff_not_found"})
#
#     check_record = fetchall__dict2dot(cursor,f'''
...
#
#
# @router.get("/sort",summary="sort for date")
# def sort_for_date(check_staff:Staff=Depends(token_check)):
#     staff_check, connection, cursor = check_staff
#     if not staff_check: JSONResponse(status_code=401, content={"status": False, "message": "staff_not_found"})
#
#     check_record = fetchall__dict2dot(cursor,f'''
